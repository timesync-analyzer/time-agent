...
std::optional<PTPMetrics> Ptp4lMessageParser::parse(const JournalEvent& event) const {
    ...
    if (sscanf(event.msg.c_str(), "ptp4l[%lf]: master offset %lld s%d freq %lld", &timestamp, &res.offset, &res.state,
               &res.freq) >= 3) {
        return res;
    }
...
std::optional<PTPMetrics> Phc2SysMessageParser::parse(const JournalEvent& event) const {
    ...
    if (sscanf(event.msg.c_str(), "phc2sys[%lf]: %*s %*s offset %lld s%d freq %lld", &timestamp, &res.offset, &res.state,
               &res.freq) >= 3) {
        return res;
    }
    res.timestamp = timestamp * 1000;
