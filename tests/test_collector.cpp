...
protected:
    Ptp4lParser ptp4lParser;
    Phc2SysParser phc2sysParser;
    PPSParser ppsParser;
};
...
TEST_F(ParserTest, ParsePtp4lEmptyMessage) {
    auto result = ptp4lParser.parseMetrics("");
    EXPECT_FALSE(result.has_value());
}
...
TEST_F(ParserTest, ParsePhc2sysEmptyMessage) {
    auto result = phc2sysParser.parseMetrics("");
    EXPECT_FALSE(result.has_value());
}
...
TEST_F(ParserTest, ParsePortEventValidEthInterface) {
    ...
    auto result = ptp4lParser.parsePortEvent(msg);
...
TEST_F(ParserTest, IsPhc2sysWaitingTrue) {
    ...
    EXPECT_TRUE(phc2sysParser.isWaiting(msg));
}
